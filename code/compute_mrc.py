"""
SUPERSEDED -- kept for provenance, do not use for new work.

This is the implementation behind the pre-camera-ready MRC run. Its stability
component is resolved through data/analysis/probing_dataset.parquet, which covers
two of the three models on 500 of the 1,200 scenarios; every trajectory absent
from that file falls through to a hardcoded 1.0 (see compute_subspace_stability
below). That left 1,987 of 3,596 trajectories with a nonzero drift rate and a
stability of exactly 1.0, which the metric's own definition forbids, and it is why
the numbers this script produces do not match the published paper.

Use compute_moral_representation_consistency.py instead. It computes stability
from the attributed framework sequence for all 3,596 trajectories, needs no
probing data, and asserts the invariant this implementation violates.
"""
